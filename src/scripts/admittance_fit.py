...
def analyze_file(
    ...
    if fit_model == FitModel.NO_LS:
        fit_results = fit_squid_model(df_modes, parameter_bounds=parameter_bounds)
        fit_name = "squid-model-fit (No Ls)"
        show_ls_in_summary = False
    elif fit_model == FitModel.WITH_LS:
        fit_results = fit_squid_model_with_Ls(df_modes, parameter_bounds=parameter_bounds)
        ...
        fit_results = fit_squid_model_with_Ls_fixed_C(
            df_modes, capacitance_pf=fixed_c, parameter_bounds=parameter_bounds
        )
...
def main(fit_model: FitModel = FitModel.WITH_LS) -> None:
    ...
    for identifier in file_list:
        ...
        if component_path is None:
            continue
        entry = analyze_file(
            component_path, modes_to_plot, parameter_bounds, fit_model, args.fixed_c
        )
        if entry:
